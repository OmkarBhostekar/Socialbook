...
class PostsViewModel @ViewModelInject constructor(
    ...
    fun getAllPosts() = viewModelScope.launch {
        val response = repository.getAllPosts(readStringFromDS(UID))
    ...
    fun getLikedBy(postId: String) = viewModelScope.launch {
        ...
        val response = repository.getLikedBy(postId,readStringFromDS(UID))
        if (response.isSuccessful)
    ...
    suspend fun readStringFromDS(key: String) : String {
        ...
        return preferences[dataStoreKey] ?: "5fc4ce20268106120c5ba490"
    }
}